...
public class SmallerEleRightGreaterEleLeft
{
// Java implementation of the approach  Function to return the sum of arr[0..index]. This function
// assumes that the array is preprocessed and partial sums of array elements are stored in BITree[]
    ...
    // Converts an array to an array with values from 1 to n and relative order of smaller
    // and greater elements remains same. For example, {7, -90, 100, 1} is converted to {3, 1, 4, 2 }
    public static void convert(int arr[], int n)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            // Arrays.binarySearch() returns index to the first element greater than or equal to arr[i]
    ...
    public static void findElements(int arr[], int n)
    {
        ...
        // Convert arr[] to an array with values from 1 to n and relative order of smaller
        // and greater elements remains same. For example, {7, -90, 100, 1} is converted to {3, 1, 4, 2 }
        convert(arr, n);

        // Create a BIT with size equal to maxElement+1 (Extra one is used
        // so that elements can be directly be used as index)
        int[] BIT = new int[n + 1];
        for(int i = 1; i <= n; i++)
            BIT[i] = 0;
